...
![](https://lh4.googleusercontent.com/out7KENUb-CTDQq3jeB8yBvxrg53YCLAPPtaGBNyGeH7ITGgi9g1PLWp8OjjRDRJYHccqO9ASNS-3W3xwmiBImR1SWj9ZlvjP_JB4gp5B45AxwGjUgqiePhE2E7_Eln3v-1L1gOmuAB5jaq9aeGx_-M)1980년 남코회사에서 출시된 Pac-Man 게임은 단순한 구조로 많은 사람들에게 사랑을 받은 플래시 게임입니다. 기본적으로 pac-man게임은 1인용 게임으로 귀신을 피해다니며 맵에 퍼져있는 점을 먹으며 점수를 쌓아가는 게임입니다.
...
첫 번째는 키 입력을 서버로 보내는 것. 이렇게 되면, 서버에서는 실제로 게임을 실행 야해야함. 이 때 다온 클라이언트들의 좌표, 방향, 점수를 클라이언트들에게 보냄. 이 때는 키가 입력됐을 때만 인터럽트 방식으로 서버에 보냄

두 번째는 키 입력으로 부터 연산된 포즈, 방향, 점수를 클라이언트에서 연산해서 서버로 그대로 보냄. 서버는 받은걸 그대~로 다른 클라이언트들에게 전송.

현재 까지 완료된 것

1. 클라이언트에서 솔로 플레이 가능(4인플 오늘 안으로 구현 예정, not server).
2. 우리 마음대로 데이터 송수신 가능.

해야될 것

1. 4인플 가능하도록(팩맨 클래스와 맵 클래스 분리)
